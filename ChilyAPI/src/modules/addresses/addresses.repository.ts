...
export class AddressRepository {
  ...
  async getUserAddresses(id: number): Promise<Address[] | Error> {
    try {
      ...
      if (!address.length)
        throw new Error("No hay direcciones para este usuario");
      return address;
    } catch (error) {
  ...
  > {
    try {
      return await this.dataSource.transaction(async (manager) => {
        const user = await this.userService.findUserById(addressDto.id);
        if (!user) throw new NotFoundException("El usuario no existe");

        const newAddress = new Address();
        newAddress.address = addressDto.address;
        newAddress.note = addressDto.note;
        newAddress.user = user;
        newAddress.location = addressDto.location;
      ...
      if (error.code === "23505")
        throw new ConflictException(
          "Ya existe una dirección para este usuario",
        );
      throw new InternalServerErrorException(error);
    }
  }
  ...
  async updateAddress(
    addressUpdate: UpdateAddressDto,
  ): Promise<Address | NotFoundException | InternalServerErrorException> {
    try {
      ...
      if (error.code == "23505")
        throw new ConflictException("Esa dirección ya fue usada");
      throw new InternalServerErrorException(error);
    }
  }
